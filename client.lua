...
local swtxt = "* Osoba uzima oruzije . *"
local mwtxt = "* Osoba vadi oruzije blizu. *"
...
-- spisak oružja koje treba izvaditi iz automobila

bigweaponslist = {	
	"WEAPON_MICROSMG", -- ovde dodajte koje oružje želite da izvadite iz automobila
	"WEAPON_MINISMG",
	"WEAPON_SMG",
	"WEAPON_SMG_MK2",
	"WEAPON_ASSAULTSMG",
...
-- Spisak poruka o oružju za malokalibarsko oružje

smallweaponslist = {
	"WEAPON_REVOLVER", -- ovde stavite oruzije koje zelite da izvadite kad god hocete
	"WEAPON_PISTOL",
	"WEAPON_PISTOL_MK2",
	"WEAPON_COMBATPISTOL",
	"WEAPON_APPISTOL",
...
-- Spisak poruka o oružju za oružje u bliskom oružju
...
function DrawText3D(x,y,z, text)
    ...
    if onScreen then
        ...
        -- Izračunajte širinu i visinu
        BeginTextCommandWidth("STRING")
        AddTextComponentString(text)
        local height = GetTextScaleHeight(0.55*scale, font)
        local width = EndTextCommandGetWidth(font)

        -- Text koj se pokazuje
        SetTextEntry("STRING")
        AddTextComponentString(text)
						...
						if duffle == true then
							if IsPedModel(playerPed,1885233650) and bigWeaponOut == false and GetVehiclePedIsIn(playerPed, false) == 0 then -- male
								...
								if GetPedDrawableVariation(playerPed,5) == 45 and GetPedTextureVariation(playerPed,5) == 0 and GetPedPaletteVariation(playerPed,5) == 0 then
									...
									Wait(1)
									drawNotification("~p~Miladinovic ~r~"..dbtxterr.."")
							...
							if bigWeaponOut == false and GetVehiclePedIsIn(playerPed, false) == 0 then
								Wait(1)
								drawNotification("~p~Miladinovic ~r~"..bwtxterr.."")
								SetCurrentPedWeapon(playerPed, -1569615261)
							end
						end
					end
